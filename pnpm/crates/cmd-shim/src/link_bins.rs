...
struct ShimSpec<'a> {
    ...
    /// Whether this run created the bin directory, so nothing can
    /// occupy [`shim_path`](Self::shim_path) yet.
    bin_dir: DirCreation,
}
...
///
/// In a bin directory this run created, that read can only report
/// `NotFound`, so the write goes first and the read never happens — the
/// exclusive create refuses anything that turned up meanwhile, and its
/// failure falls back to the read. Trying the write first anywhere else
/// would add a failed create to every shim already in place, which is
/// what an ordinary reinstall is made of.
